...
ALTER TABLE policy_rules ADD COLUMN src_node_id uuid REFERENCES nodes (id) ON DELETE CASCADE;

-- ⚠ ON DELETE CASCADE, and it is the right action HERE: a grant whose source node is gone is not a grant
...
CREATE INDEX IF NOT EXISTS policy_rules_src_node_id_idx ON policy_rules (src_node_id) WHERE src_node_id IS NOT NULL;
...
ALTER TABLE policy_rules ADD CONSTRAINT policy_rules_src_check CHECK (
    (src_kind = 'group' AND src_group_id IS NOT NULL AND src_user_id IS NULL AND src_site_id IS NULL AND src_cidr IS NULL AND src_node_id IS NULL)
 OR (src_kind = 'user'  AND src_user_id  IS NOT NULL AND src_group_id IS NULL AND src_site_id IS NULL AND src_cidr IS NULL AND src_node_id IS NULL)
 OR (src_kind = 'site'  AND src_site_id  IS NOT NULL AND src_group_id IS NULL AND src_user_id IS NULL AND src_cidr IS NULL AND src_node_id IS NULL)
 OR (src_kind = 'cidr'  AND src_cidr     IS NOT NULL AND src_group_id IS NULL AND src_user_id IS NULL AND src_site_id IS NULL AND src_node_id IS NULL)
 OR (src_kind = 'agent' AND src_node_id  IS NOT NULL AND src_group_id IS NULL AND src_user_id IS NULL AND src_site_id IS NULL AND src_cidr IS NULL)
);
