...
-- Existing memberships are conservatively treated as durable manual/legacy access.
...
CREATE INDEX membership_access_sources_lookup_idx
    ON membership_access_sources (org_id, user_id);

INSERT INTO membership_access_sources (org_id, user_id, source_type, source_key)
SELECT org_id, user_id, 'manual', 'legacy'
FROM memberships;

CREATE OR REPLACE FUNCTION membership_access_source_on_insert()
RETURNS trigger AS $$
BEGIN
  INSERT INTO membership_access_sources (org_id, user_id, source_type, source_key)
  VALUES (NEW.org_id, NEW.user_id, 'manual', 'legacy')
