...
func TestWindowsBypassFlagRequiresGuard(t *testing.T) {
	...
	hasGuard := strings.Contains(s, `"full_tunnel_unsupported"`) && strings.Contains(s, `"win32"`)
	if hasFlag && !hasGuard {
		t.Fatal("S6.10 coupling: the dev bypass flag is present without the win32 full_tunnel_unsupported guard — remove the flag and guard TOGETHER, never the guard alone")
	}
}
