...
| **"Subnet advertisement queue"** (Sites queue title) | **DIFFERENT TERM** | Wireframe titled panel *"Subnet advertisement queue"*; domain model and API call them *"Pending subnet approvals"*. Managed on Sites screen | S14.7 |
...
| **Pending Subnet Queue on Routed Ranges** | **CUT FROM ROUTED RANGES** | Cutting the grid leaves Routed Ranges as a read-only projection of approved CIDRs (`/routed-ranges`). Pending subnet approvals are managed on Sites (`/api/v1/organizations/{orgId}/site-subnets/pending`) | S14.7 |
