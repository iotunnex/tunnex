...
| **Direct-to-`main` pointer pushes bypassing the 3 required checks — RUNNING COUNT: 6** | **EPIC 14 close** | Authorized under **Ruling 2** (a process/docs correction whose value is immediate lands on `main` directly) and **reported every single time** — but ONE PER MERGE is a standing MECHANISM, not an exception, and a mechanism that bypasses `gates` / `client (macos-latest)` / `client (windows-latest)` deserves a decision rather than a habit. **The pointer is docs-only, so the bypass buys ~8 min it would otherwise wait for; the ruling due at epic close is whether this STAYS the mechanism or the pointer MOVES somewhere that needs no bypass at all** (a release note, a generated file, or after the `gates` split makes a docs-only run ~1.7 min and the bypass nearly worthless). ⛔ **The split changes the arithmetic of this row, so rule them together** | S14.5→S14.10, one per merge | authorized, uncounted until now |
