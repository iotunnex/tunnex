...
`edition_required`. **FOUNDER-RULED (a): the gate was the bug. DELETED in `42cbe4b`**, and the sweep that
followed found the inverse defect too — `ENTERPRISE_PATHS` was missing three genuinely-gated device endpoints
because the census matched `(enterprise)` alone while their summaries read `(…, enterprise)`. **Verified on a
live stack: an open-edition run rendered Sites.**
