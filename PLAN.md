...
**CURRENT (2026-07-18): EPIC 8 (now 5 stories) — S8.3 (site management UI) MERGED (PR#29, sha `e3ba76a`), 3 of 5. NEXT: S8.4 (DNS forwarding) commit-one — its own fresh sitting; PREREQUISITE before S8.4's paper = the two-part verify-item (enterprise resource-CIDRs→AllowedIPs? / open LAN-route surface today?). Cross-cloud site-to-site demo (AWS Sydney↔Azure West US) staged, runs post-merge (`walk-artifacts/cross-cloud-demo/demo-script.md`).**
