...
**S15.2 COMMIT-ONE (2026-08-04) — content tip `3c29dd86` pre-merge. PAPER ONLY; NO PRODUCT CODE.**
...
**FOUR DECIDE-ITEMS HELD (§7), NO PRODUCT CODE UNTIL DISPOSITIONED**, and ⛔ **D25 RANKS FIRST BECAUSE THE
OTHER THREE ARE SHAPED BY IT:** **D25** what happens when an agent has no owner or its owner is deactivated
(refuse at use · refuse at ENROLMENT never at use · degrade-and-flag; 2+3 compose) — **S15.1's refusal was
cheap because a human at a screen fixed it in minutes, and an agent's is not** · where an agent's owner comes
from · `devices` row or its own kind · **D24** one constructor with a discriminator or a second with the
census RE-RUN (the property was measured once and is not self-renewing). ⚠ D25 should probably be ruled
alongside **D23**, and the paper does NOT assume the same answer.
