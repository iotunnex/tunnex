...
**CURRENT (2026-07-19): EPIC 8 (8 stories) — S8.4 (cross-site DNS) MERGED (PR#31, rebase-ff `bb844f6`) → EPIC 8 is 5 of 8 (S8.1·S8.2·S8.2c·S8.3·S8.4 merged). S8.4 review arc CLOSED (5 rounds → round-3 HALT reduce-by-removal, ripped the resolver release-rider; two laws minted: shared-territory ownership + dormant-machinery addendum) + box-walk 6/6+liveness GREEN on a live cross-cloud pair (`walk-artifacts/S8.4/walk-record.md`). HUB behind-host forwarding VERIFY-THEN-FIX CLOSED verified-no-agent-fix-needed (`walk-artifacts/hub-forwarding-verify/`; forward chain exonerated, WF-4 present+firing, no manual rule, fixture-fidelity last-quadrant leg live-proven via a site→site grant). S8.5 (open-edition routed subnets) IN PROGRESS on branch `story/S8.5-routed-subnets` — commit-one RULED (`docs/S8.5-decisions.md`: D1-D8 confirmed, reuse-site-subnets one-model, D1 one-screen-affordance requirement, D5 two-sided twin-golden) + **Slice 1 DONE (crash/owner-loss resolver sweep — the D4 ordering precondition; the dormant-machinery law's first scheduled compliance; rides the dead-man release OUTSIDE the lock, deadlock-proof red, helper race-clean)** + **Slice-2 mid-slice FORK surfaced & RULED: Option A (volatile-routes channel + live-apply — the client config is two-tier: stable identity never re-fetched + volatile routes polled like revocation)**. **NEXT SESSION OPENS WITH: build Slice 2a — the helper `set_allowed_ips` verb + both-platform live-apply (uapi `update_only=true replace_allowed_ips=true` via dev.IpcSet, NO bounce, + OS-route full-sweep) + the full red set (dispatch-route · tunnel-untouched · smuggle-reject · no-bounce[handshake+transfer survive] · full-tunnel-no-op · update_only-asserted). Then 2b (CP device-authed routed-ranges channel) → 2c (client RoutedRangesMonitor poll+canonicalized-merge, fail-static, full-sweep) → 2d (D1 one-screen affordance + D5 twin-goldens) → review → cross-cloud walk (7 legs + behind-hub inheritance + crash-sweep leg).** 2a is privileged live-WireGuard + route-table code — build it FRESH, not tail-of-turn (the S8.4-arc lesson). Then S8.6 hub-HA → S8.7 (CIDR-source /32 + expired-grant conntrack-flush) → epic-close walk. Deferred (named triggers): S8.4b Windows NRPT (Slice 4) + crash-sweep now landing in S8.5 Slice 1.**
