...
**S15.3 — THE AGENT SURFACE + THE UI PASS — PR #90, content tip `224aa883` (pre-merge; post-merge sha to be
filled in as a docs correction on `main`).** ⚠ *Re-pointed once: the first checkpoint named `dac3382f` and
then CI red-lined, so a fix landed after it. Re-pointed in the same breath rather than at merge time — the
rule is fine and the SEQUENCING is what slips.* `docs/S15.3-decisions.md`, `docs/rule-validity-matrix.md`,
