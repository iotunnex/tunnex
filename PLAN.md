...
**CURRENT (2026-07-18): EPIC 8 (now 6 stories) — S8.3 MERGED (PR#29, `e3ba76a`); S8.2c (gateway zero-touch) IN FLIGHT, inserted ahead of S8.4. Branch `story/S8.2c-gateway-zero-touch` (pushed) — Slice 1 at D1+D2 DONE (`1e4f782` D1 mesh symmetric forward, `af9bf37` D2 CP LocalSubnets + agent src-hint), both gated green. NEXT = S8.2c Slice 1 D3 (FRESH SITTING).**
- **S8.2c re-entry — resume at D3:** the bridge-trapped / unreachable-advertised-subnet **health kind** (B2 pattern, loud/operator-visible, never silent). **Must catch the reassuring-green shape** the cross-cloud demo hit: wg0 alive + handshake fresh, yet the advertised subnet is unreachable from the container's netns (bridge mode). **Groundwork already laid:** `siteRouteSrc` (reconcile pkg) returns the `hadSubnets && !matched` signal; `ApplyRoutes` already logs `site_route_src_unresolved`. D3 = wire that signal → agent sink → report → CP caps → new `KindSiteSubnetUnreachable` + `degradedKind` precedence → OpenAPI enum + regen → web `healthview` badge → reds (~9 files). Full multi-layer, deliberately deferred to a fresh sitting (not a tail-end cram).
