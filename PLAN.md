...
**CURRENT (2026-07-19): EPIC 8 (8 stories) — S8.4 (cross-site DNS) MERGED (PR#31, rebase-ff `bb844f6`) → EPIC 8 is 5 of 8 (S8.1·S8.2·S8.2c·S8.3·S8.4 merged). S8.4 review arc CLOSED (5 rounds → round-3 HALT reduce-by-removal, ripped the resolver release-rider; two laws minted: shared-territory ownership + dormant-machinery addendum) + box-walk 6/6+liveness GREEN on a live cross-cloud pair (`walk-artifacts/S8.4/walk-record.md`). HUB behind-host forwarding VERIFY-THEN-FIX CLOSED verified-no-agent-fix-needed (`walk-artifacts/hub-forwarding-verify/`; forward chain exonerated, WF-4 present+firing, no manual rule, fixture-fidelity last-quadrant leg live-proven via a site→site grant). NEXT = S8.5 (open-edition routed subnets) commit-one, then S8.6 hub-HA → S8.7 (CIDR-source /32 precision + expired-grant conntrack-flush) → epic-close walk. Deferred with named triggers: S8.4b Windows NRPT + the crash/owner-loss resolver sweep (client-resolver-path-live trigger, ordering precondition in PLAN).**
