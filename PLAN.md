...
**AUTHORITATIVE CURRENT (2026-08-08): GET.TUNNEX.IO INSTALLER + BOOTSTRAP RACE CLOSURE — MERGED via PR #100, content tip `7b0ff842` (`b8c4d471` pre-merge). Earlier `CURRENT` entries below are historical.**
...
4. **MAIL FAILURE WAS REPORTED AS SUCCESS.** `mail.New()` with no SMTP host returned a mailer that LOGGED
   ...
   is no longer logged — it carried invitation and reset links into shipped logs. **Mailpit removed from
