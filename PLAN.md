...
**IN PROGRESS (2026-07-24): EPIC 9 / S9.1 OpenVPN — branch `story/S9.1-openvpn` (NOT merged; decision-first, `docs/S9.1-decisions.md`).** Commit-one + all six D-S9.1-* + D-S9.2-* + D-S9.3-* + D-S9.4-MODEL RULED. **Built + gated (both editions + node), on the branch:** Slice 1 (B1 boundary in code — compiler transport-agnostic + chain address-keyed; WIRE proof OWED, trigger = the S9.1 box-walk) · Slice 2 (`ovpnca` separate client CA + PKI + issuance; migration 0041 `ovpn_client_certs`) · Slice 3 four parts (mesh relaxation: `ip tunnex` interface-SET keying + Docker-USER pool & localSubnets per-interface + route site-link-noted — anti-spoof + zero-config reds green; `ovpnserver.Manager` server-lifecycle+CCD, unit-proven, UNWIRED — the 6 live-wiring preconditions named in the paper) · Slice 4a (`ovpnca.IssueServer` server-auth leaf, role separation) · 4b-core (`ovpn.BuildProfile` .ovpn assembler, one-time-by-construction) · **4b-model** (D-S9.4-MODEL: `devices.transport` tag migration 0042 — FILTER-only, compiler transport-blind, Slice-1 checkpoint re-verified; cap+pool+revocation inherited via the shared Create/Revoke path). **Shas: `9074323`/`f75725c`/`13a8a59` (Slice 3) · `e205223` (ovpnserver) · `c5dc6fd` (4a+4b-core) · `8465a66` (4b-model) · `ef7b8f4`/`8d27cce`/`5e2b1af`/`5c11bbb`/`53fd634` (commit-one+S1+S2).** **4b-wiring SERVICE layer done** (OVPN-create fork at ONE seam `b7a3313` — no WG keypair/peer, pool+/32-in-artifact, migration 0043 keyless-device index; `ExportProfile` orchestration `acd6506` — issue+assemble+serial-fingerprint) · **D-S9.5-OPTIN RULED** (OVPN unlock-then-opt-in, OFF-by-default org-level + per-gateway, binary-unconditional, disable≠revoke full-sweep). **4b-wiring BACKEND HTTP done** (export endpoint `POST /organizations/{orgId}/ovpn-profiles` — create-fork + ExportProfile, returns once, fingerprint-audited; migration 0044 org.ovpn_enabled opt-in gate `opt_in_required`; LAZY CA per D-S9.5-OPTIN(a) — LoadOrCreate on first export never at boot; commit `c0fa10d`). **NEXT = 4b-wiring WEB one-time ceremony** (OneTimeSecretModal + hide OVPN type when !ovpn_enabled) **+ 4c** (roster ListActiveOVPNDevicesForNode → out-of-hash desired-state field → agent ovpnserver.SetDesired; DesiredState-exclusion red + indistinguishable-/32 graduation). Owed: handler opt-in-gate red (gate logic simple, authwalk covers auth) (coupled: the export path creates the device the roster carries): the OVPN-create FORK at ONE seam (D-S9.4-MODEL — no WG keypair/peer, but pool+/32-in-artifact; the fork red spec'd in the paper) + the export endpoint (OpenAPI → handler → `ExportProfile`: create OVPN device → `Issue` cert → resolve gateway endpoint → `BuildProfile` → return once, audit the cert-serial fingerprint never the material) + the one-time web ceremony (`OneTimeSecretModal` pattern, loss path shown = revoke-and-reissue never re-download) + 4c roster → out-of-hash desired-state field (the indistinguishable-/32 red graduating unit→wiring). **THEN checkpoint → 4d** (live wiring in precondition order + refuse-loudly health kind for enabled-without-certs, NEVER crash-loop) → **4e box-walk** on the live cross-cloud env (discharges Slice-1's named SUBSTITUTE — real OpenVPN Connect client, enforcing+zero-grants DROPPED byte-for-byte, then grant→flows). Pre-existing bindings intact (ZT-coverage / revocation-parity / S9.3-lock). Slice 5 (revocation full-sweep + CRL reconcile) after Slice 4.
