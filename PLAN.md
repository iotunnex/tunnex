...
**CURRENT (2026-07-19): EPIC 8 (8 stories) — S8.4 (cross-site DNS) MERGED (PR#31, rebase-ff `bb844f6`) → EPIC 8 is 5 of 8 (S8.1·S8.2·S8.2c·S8.3·S8.4 merged). S8.4 review arc CLOSED (5 rounds → round-3 HALT reduce-by-removal, ripped the resolver release-rider; two laws minted: shared-territory ownership + dormant-machinery addendum) + box-walk 6/6+liveness GREEN on a live cross-cloud pair (`walk-artifacts/S8.4/walk-record.md`). NEXT = HUB behind-host forwarding VERIFY-THEN-FIX (urgent fast-follow — the manual `iptables FORWARD` accept the founder needed on the AWS hub; verify-cited → agent-owned symmetric accepts, UI-raw-iptables REFUSED), then S8.5 → S8.6 hub-HA → S8.7 CIDR-source → epic-close walk. Deferred with named triggers: S8.4b Windows NRPT + the crash/owner-loss resolver sweep (client-resolver-path-live trigger, ordering precondition in PLAN).**
...
**S8.7 — CIDR-scoped rule sources — REGISTERED (founder-directed, live-walked 2026-07-19; first named demand = founder). Sequenced AFTER S8.6.** **The gap (founder hit it live):** the Access rule modal cannot express a specific-source-IP → specific-dest-IP for behind-gateway hosts — the **Source** type offers **Group/User only**, and resources are **destination-only**. Founder attempted `172.31.17.64/32 → 10.0.0.4/32` (behind-hub host → behind-spoke host) and had no way to name the source. **PLACEMENT (argued):** standalone S8.7 IN EPIC 8, not folded into a future generic policy-granularity story — the demand is behind-gateway-host↔behind-gateway-host, intrinsically a site-networking scenario, and `src_kind='cidr'` rides the **site-src `saddr` compiler path proven in S8.2**; folding it out would divorce it from the exact machinery it mirrors + defer a founder-hit gap. **Commit-one sketch:** `src_kind='cidr'` on the existing enum seam (`policy_rules.src_kind`) · compiler **`saddr` emission mirroring the site-src path** (S8.2 already proved the saddr class) · **version/hash classification argued per the D2-checklist row — enforcement-significant, LIKELY rides the existing v5 since site-src already proved the saddr class, but VERIFY don't assume** (a new saddr-source that changes no wire field but changes the match set) · modal **Source dropdown gains Resource/CIDR, symmetric with the destination** side · **one validator + typed refusals** per convention (the CIDR validated once, no JS re-check). **Sequencing stays honest:** after S8.6 — S8.4's merge, the hub-forwarding fix, S8.5, and HA do NOT move; this registers now so it can't be lost and builds in its slot.
