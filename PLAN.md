...
**CURRENT (2026-07-23): EPIC 8 COMPLETE (8/8 MERGED) + v0.2.0 CUT + SMOOTH WALK PASSED. Fix-forward advancing on `walk/epic8-smooth-walk` (docs+fix branch, NOT merged). DONE: WF-B COMPLETE (all tiers + health-surface review + F1/F2 fold) · WF-C Layer 1 hotfix SHIPPED · feature requests registered · WF-A paper COMPLETE+RULED+VERIFIED · **WF-A CP TIER BUILT (slice 1a `e18d4df` widenedDevicePeers [DB-red PASS: primary /32, standby warm] + slice 1b `0807f3a` activeHubDialFrom pure primitive [red: primary/standby→active-primary, spoke→not-derived, promotion-follows]).** NEXT = WF-A slice 2 (client tier — HELD for fresh session per the kill-switch discipline).** Branch tip carries: WF-B slices 2a/2b/2c (`the ONE liveness derivation` → CP verdict → API/codegen → web badge) + F1/F2 fold (`silence yields nothing` guard); WF-C Layer 1 (`WGBackend.Close` → `ip link del`, idempotent, main defers it; red present→del/absent→idempotent; test-node green). **WF-A ready to build (paper `docs/WF-A-device-rehoming-decisions.md` fully ruled): CP-driven re-homing, D-WFA-5 RULED (C) — identity STAYS (node_id), dial endpoint DERIVES from deriveActive's HEAD (active primary), NOT a mint-path change. D-WFA-5b peer-hosting VERIFIED (cited devices.sql:164 `WHERE node_id=$1` → device peer scoped to assigned node only → (C) is a HALF-FIX): the COMPANION rides this story = device peers compiled onto EVERY hub-set member (warm on standbys like site-links; /32 AllowedIPs rides the active-primary recompile). D-WFA-4 full-tunnel CP-endpoint kill-switch carve-out IN v1. WF-A BUILD SLICES: (1 CP — DONE) widenedDevicePeers + activeHubDialFrom, committed+tested; (2 client — NEXT, HELD fresh) wire activeHubDialFrom to mint buildConfig (devices/service.go:264 endpoint+pubkey → active primary) + the dial-endpoint on the EXISTING routed-ranges channel per D-WFA-6 (one-truth, NOT a 3rd poller; verify device-scoping — routed-ranges is org-scoped, the dial endpoint is device-scoped) + client endpoint monitor (re-home on change) + helper `set_endpoint` verb (no-bounce like set_allowed_ips); (3 helper — kill-switch, undiscounted) full-tunnel CP-endpoint carve-out (one named pass rule mirroring the WG-endpoint host-route, backend_darwin.go:301-323, D-WFA-4). Slice 3 = kill-switch mod, the most-verified component (2 platforms, live-pcap, S6.3 paper, S6.7 arc) — earns a fresh session, undiscounted review after. NOTE for build: reconcile.go Peer.SiteLink governs endpoint-comparison — device peers are endpoint-BLIND today (roam); the re-home endpoint semantics must account for this. REDS: promotion→device's next config carries promoted-hub endpoint (one-truth) · device on hub-set member appears in promoted hub's peer set (dial handshake completes) · identity-stable (node_id/audit/revocation unchanged by promotion — (C) didn't smuggle (A)) · full-tunnel+kill-switch→CP reachable, all else dropped · the walk's clocked fixture BOTH modes. Then targeted review (peer-model + kill-switch, UNDISCOUNTED) → WF-C Layer 2 paper (failover-liveness: zombie-hub stale-policy-enforcement hazard; (b)container-netns + (c)two-signal-liveness layered, (c) must state which signal wins on disagreement) → COMBINED box-walk (one hub-kill, two acceptances: WF-B badge transit-healthy+named-demoted-peer WHILE WF-A device re-homes on the stopwatch, both tunnel modes).** Registered post-epic (papers after box-walk, `docs/POST-EPIC8-feature-requests.md`): port-scoped resources (lead-verify: UI-only vs model+compiler) + per-rule enable/disable toggle (disabled=as-if-absent under default-deny). Devices-on-SPOKES re-home gap stays DEFERRED (no promotion event fires; (C)'s derivation = the seam a future fix extends). WF papers all on the walk branch: `docs/WF-A-device-rehoming-decisions.md` (RULED — CP-driven re-homing, promotion-triggered/audited/failback-symmetric; D-WFA-4 full-tunnel CP-endpoint kill-switch carve-out IN v1; D-WFA-0 verified split-independent/full-rides-tunnel, cited) · `docs/WF-B-site-link-badge-decisions.md` (RULED — consume the ONE liveness derivation, flat KindInput fields, rank-by-transit-consequence, agent-bool D-WFB-1b retire-or-vestigial). **WF-B SLICE 1 DONE (`125ff7b`): `deriveMemberLiveness` shared pure function (the D-WFB-1 one-liveness-truth); failover controller refactored to read it (suite green = behavior-preserving); grep-red green (only `<failoverStaleWindow` inside the shared fn); red `TestDeriveMemberLivenessSharedTruth` incl. the demoted-dead case. NEXT = WF-B SLICE 2 (health surface consumes it, six-item scope): (1) SiteTopoBatch loads hub-set+node_peer_status→deriveMemberLiveness once; (2) replace `caps.SiteLinkStale` (service.go:1364) at the spoke-headline seam — active-primary-stale=headline, demoted-member-stale-while-primary-fresh=subordinate; (3) NEW subordinate-site-link-note on PolicyHealth (peer name+demoted) INDEPENDENT of the headline Kind→KindInput flat fields→OpenAPI/codegen→web; (4) degradedKind precedence (D-WFB-3); (5) reds: walk's exact state (demoted-dead→healthy headline+named line) + INVERSE (active-primary-stale→headline degraded, the reassuring-green-at-precedence guard) + two-truths (badge follows controller); (6) web badge names peer + `(demoted)` + subordinate render. Agent-bool D-WFB-1b decided in-build (retire from payload or vestigial-until-agent-vN, per the dormant-data guard). Then gates+reds → targeted review on the health surface (S7.4b/S8.2, undiscounted) → WF-C characterize (read-only, rides the gap; hotfix unless kill-switch-touch) → WF-A build → its review (peer-model+kill-switch, undiscounted) → COMBINED box-walk: ONE hub-kill, TWO acceptances (WF-B badge shows transit-healthy+named-demoted-peer WHILE WF-A device re-homes on the stopwatch, joining the 4m48s as the demo's second number).** Train landed ff-exact on Pawan's words: S8.5 `8eabec4` → S8.6 `7083889` (hub HA + S8.6b transit + A3b device-pool v6) → S8.7 `bb05df5` (cidr-src + conntrack-flush); every merged sha == its CI-tested sha. **v0.2.0 tagged @ `7a7c331`** (5 ghcr images; node-agent digest `sha256:de8c9cefb614…22f114`). **Pre-walk deploy hotfix main `fa195ae`**: docker-compose forwards `TUNNEX_NODE_AGENT_IMAGE` (the .env digest pin silently no-op'd). **SMOOTH WALK from merged main PASSED** (`walk-artifacts/S8.6/SMOOTH-WALK-record.md`, branch `walk/epic8-smooth-walk` = `1885cfb` record + `ebc223c` WF-A paper + fabric HA note; NOT merged — docs branch awaiting fix-forward builds). PROVEN on shipped v0.2.0: A3b device→remote-site ZERO-TOUCH cross-cloud · ZTNA default-deny+scoped-grants+conntrack-flush (delete-immediate + expiry-autonomous) · hub-HA warm standbys + failover PROMOTION + FAILBACK (cycled v2→v6). **FIX-FORWARD BACKLOG (founder-ruled dispositions):** (1) **WF-A device re-homing (client HA)** = own story; paper `docs/WF-A-device-rehoming-decisions.md` held. **NEXT-SESSION FIRST ACTION: verify D-WFA-0 control-path-independence — when a device's data tunnel is dead, can it still reach the CP API for a re-homed config? Trace the client control channel, CITE the code path.** Independent→CP-driven re-homing (founder lean, rides promotion→compile→push, observe-never-vote preserved); through-tunnel→forces client-side multi-endpoint. Bring cited answer → founder rules D-WFA-1 mechanism → complete paper → build. (2) **WF-B site-link-badge** = COMMIT-ONE (ONE paper: naming + subordination share the CP per-peer computation). LEAD DECIDE-ITEM (founder strong prior): WF-B per-peer health MUST consume the failover controller's EXISTING liveness derivation (spoke-observed freshness ⋂ hub-set membership), never a 3rd freshness (two-truths class). Deck-D badge-names-which-peer folds in. Red: promotion-in-effect + demoted-dead peer → transit-healthy headline + named-peer-down line item, both distinct. (3) **WF-B(3) VPC-fabric = DONE as docs** (`ebc223c` fabric panel HA note): behind-host HA needs cloud-side route failover (route-table health-check/GWLB/UDR); overlay fails over, VPC route to ENI is static (zero-touch boundary, BY DESIGN). (4) **WF-C orphaned wg0** (`docker stop` leaves wg0 in host netns → zombie hub, failover-blind to agent-death): characterize next session — restart-lifecycle-residue=hotfix; kill-switch-touch=decision-first. (5) **WF-E/F + harvest verdicts (group-membership-no-UI, resource-no-port-field)** = DEFER to a post-epic UI-polish story (spec = founder's walk-ranked irritation list); site-link-down-names-peer folds into WF-B. **Failover test method (proven): `docker stop tunnex-node && sudo ip link del wg0` on the HOST** (wg0 in host netns; docker-stop alone leaves the data plane alive). Promotion after N=3 stale ticks, failback after M=5 fresh (M>N). **DEFERRED (named triggers, carried):** devices-on-SPOKES cross-site · PD-3 residual (single/non-site device↔device Docker-dark) · IPv6 pool · S8.4b Windows NRPT · WF-7 site-rule editor · SSO-exempt wire proof · MFA break-glass. **ENTITY-GATED:** Gatekeeper/S6.5b signing (confirmed-live on the walk; xattr+adhoc-codesign is the dev workaround). Prior: S8.4 (cross-site DNS) MERGED (PR#31, rebase-ff `bb844f6`). S8.4 review arc CLOSED (5 rounds → round-3 HALT reduce-by-removal, ripped the resolver release-rider; two laws minted: shared-territory ownership + dormant-machinery addendum) + box-walk 6/6+liveness GREEN on a live cross-cloud pair (`walk-artifacts/S8.4/walk-record.md`). HUB behind-host forwarding VERIFY-THEN-FIX CLOSED verified-no-agent-fix-needed (`walk-artifacts/hub-forwarding-verify/`; forward chain exonerated, WF-4 present+firing, no manual rule, fixture-fidelity last-quadrant leg live-proven via a site→site grant). S8.5 (open-edition routed subnets) IN PROGRESS on branch `story/S8.5-routed-subnets` — commit-one RULED (`docs/S8.5-decisions.md`: D1-D8 confirmed, reuse-site-subnets one-model, D1 one-screen-affordance requirement, D5 two-sided twin-golden) + **Slice 1 DONE (crash/owner-loss resolver sweep — the D4 ordering precondition; the dormant-machinery law's first scheduled compliance; rides the dead-man release OUTSIDE the lock, deadlock-proof red, helper race-clean)** + **Slice-2 RULED Option A (two-tier config: stable identity never re-fetched + volatile routes polled like revocation). SLICES 2a/2b/2c DONE (Option A functionally END-TO-END): 2a helper `set_allowed_ips` live-apply (uapi update_only+replace_allowed_ips, no bounce, both platforms) · 2b CP `GET /routed-ranges` channel (revocation-poll auth class, ranges-only, approved-only, canonical, empty-first-class) · 2c client `RoutedRangesMonitor` (fail-static, no-churn, full-sweep, split-tunnel-only, per-session recreate). All gated green, ~20 reds.** **MINT-PATH FORK RULED (A): mint stays STABLE-CORE-ONLY; the poll covers new devices too (the baked config NEVER contains a routed range, for any org, ever → D5 golden categorical + full-sweep trivially correct).** **NEXT SESSION OPENS WITH: build Slice 2d FRESH (founder-facing UI — tail-of-turn rule applies undiscounted): (1) RouteLAN composite = RegisterSite+BindNode+AddSubnet+ApproveSubnet composing the four EXISTING service methods → four constituent audit events by construction; red = byte-identical DB state (RouteLAN vs the four manual steps); endpoint `POST /routed-lans` (site:manage). (2) IMMEDIATE-FIRST-POLL fix (the ruling-A condition): RoutedRangesMonitor `start()` must fire an immediate poll then the 30s cadence (currently schedules the first after baseMs — shrink the new-device gap to seconds); red = start→one fetch before any interval. (3) D5 two-sided twin-goldens: gateway `CanonicalHash` unchanged by a declaration + `allowedIPsFor` byte-identical UNIVERSALLY (mint stable-core-only). (4) subnet-conflict teaching text on the `subnet_not_disjoint` refusal. (5) one-screen "Route a LAN through this gateway" affordance (gateway card / empty-Sites, open-edition reachable; full ceremony untouched). Then Slice 3 (resolver handoff — un-gates S8.4 client DNS) → Slice 4 (NRPT) → Slice 5 (metrics + WF-1 + stale-button) → review → cross-cloud walk (7 legs + behind-hub + crash-sweep legs).** Then S8.6 hub-HA → S8.7 (CIDR-source /32 + expired-grant conntrack-flush) → epic-close walk. Deferred (named triggers): S8.4b Windows NRPT (Slice 4) + crash-sweep now landing in S8.5 Slice 1.**
